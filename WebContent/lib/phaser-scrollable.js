var ScrollableArea = function(x, y, w, h, params,createMask,xMask,yMask   ) {
	...
	this.maskGraphics.beginFill(0x00ffff);
	

	if(createMask){
		this.maskGraphics.drawRect(xMask, yMask, w, h );
	}
...
if(key1.isDown){return;}
		...
		if (this.settings.horizontalScroll) {
			//console.log("settings "+this.settings.horizontalScroll);
		...
		//console.log("verticalScroll "+this.settings.verticalScroll);
...
/**
* Active Scroll Vertical  to limit
*/
ScrollableArea.prototype.LimitScrollVertical = function () {

	if(this.useLimitVertical){
		if(this.y > this.limitScrollVertical){
			this.y = this.limitScrollVertical;
		}
	}
...
* Active Scroll Vertical  to limit
*/
ScrollableArea.prototype.activeLimitScrollVertical = function (useLimitVertical,limitScrollVertical) {

		this.useLimitVertical  = useLimitVertical;
		this.limitScrollVertical = limitScrollVertical;
	

}

/**
* Kill mask when press button
*/
ScrollableArea.prototype.kill = function () {
	if(this.maskGraphics != undefined){
		this.maskGraphics.kill();
...
* Set Press Down
*/
ScrollableArea.prototype.PressedDown = function (pressedDown) {
			...
			if (Math.abs(game.input.x-this.inputX) <= this.settings.clickXThreshold && Math.abs(game.input.y-this.inputY) <= this.settings.clickYThreshold) {
				for (var i=0;i<this.children.length;i++) {
					if (this.getChildAt(i).getBounds().contains(game.input.x, game.input.y) && this.getChildAt(i).inputEnabled && this.getChildAt(i).events.onInputUp.getNumListeners() > 0) {
						this.getChildAt(i).events.onInputUp.dispatch(this,this.getChildAt(i),false);
...
* Set Position 
*/
ScrollableArea.prototype.UpdatePosition = function(x, y) {
	this.x = x;
	this.y = y;
}
...
ScrollableArea.prototype.mouseWheel = function (event) {
	...
	//	console.log("Mouse Wheel " +this.y);
